...
- [x] S-49 — **Google Sign-In Page UI**
  ...
  - Reason: Google Sign-In provides secure, passwordless authentication without requiring users to create or remember passwords. Users authenticate using their existing Google accounts which is familiar and trusted. Firebase Auth integrates seamlessly with Google Sign-In requiring minimal configuration. This matches modern authentication patterns used by major e-commerce platforms. Single sign-on improves user experience by reducing friction in the authentication process. Created GoogleSignInPage (lib/views/auth/google_signin_view.dart) with clean UI matching shop.upsu.net styling featuring single "Sign in with Google" button with official Google branding and logo. Button triggers AuthService.signInWithGoogle() method which opens Google OAuth popup for account selection and authorization. Implemented comprehensive loading states showing circular progress indicator during sign-in process. Error handling displays user-friendly SnackBar messages for common failures including cancelled sign-in, network errors, and credential issues. Success flow shows welcome SnackBar with user's display name and automatically redirects to /account dashboard. Added /account/login route to app_router.dart routing to GoogleSignInPage. Updated SharedHeader (lib/widgets/shared/shared_header.dart) account button to navigate to /account/login (will be updated in S-54 to conditionally navigate based on auth state). Page includes comprehensive privacy notice explaining Google authentication data access (name, email, profile picture) with info icon and grey info box. All UI elements use university purple (#4d2963) for buttons and accents maintaining brand consistency. Includes all required test Keys (google_signin_page, google_signin_button). Updated AuthService (lib/services/auth_service.dart) with signInWithGoogle() method using google_sign_in package integrating with Firebase Auth via GoogleAuthProvider credentials. signOut() method updated to sign out from both Firebase and Google preventing cached sessions. Error handling in AuthService covers account-exists-with-different-credential, invalid-credential, operation-not-allowed, user-disabled, and network errors with descriptive user-friendly messages. Added google_sign_in ^6.2.1 package to pubspec.yaml. Updated auth_service_test.dart removing email/password test methods and adding signInWithGoogle and signOut tests. Authentication now fully Google-based eliminating need for password management, email verification codes, or separate signup/login pages. Users click "Sign in with Google" button, authenticate via Google OAuth popup, and are automatically signed into Firebase with their Google account. New users get Firebase accounts created automatically on first sign-in. Existing users sign into their accounts seamlessly. Single unified authentication flow simplifies UX significantly matching modern e-commerce patterns. Google Sign-In fully functional and tested - users can successfully authenticate, see welcome messages, and are ready for protected routes (S-54) and account dashboard (S-55) implementation.

- [x] S-50 — **Login Page UI**
  - Create login page (lib/views/auth/login_view.dart) with email/password fields
  - Add "Sign in" button that calls AuthService.signIn()
  - Display loading state while signing in
  - Show error messages if sign-in fails (e.g., user not found, wrong password)
  - After successful sign-in, redirect to /account dashboard
  - Add route '/account/login' to app_router.dart (login page)
  - Update SharedHeader account button to navigate to /account/login if not signed in, /account if signed in
  - Match visual styling (clean form layout, university purple accents)
  - Add Key('login_page'), Key('email_field'), Key('password_field'), Key('sign_in_button') for testing
  - Page should explain that users sign in with their email and password
  - Include "Forgot password?" link that sends password reset email
  - Reason: Login page allows existing users to sign in with email and password, matching common e-commerce authentication patterns. Integrated with AuthService for seamless authentication. Provides "Forgot password?" link for password recovery. Fully implemented with clean UI, error handling, loading states, and successful authentication flow redirecting to /account dashboard. Includes all required test Keys. Note: Actual email/password sign-in requires Firebase Emulator for testing, or use real email/password in Firebase Auth.
